...
### After API is implemented — validate with tester-api

Once the API is running (local or staging), spawn `tester-api` to validate the actual implementation matches the design:

```
Agent(subagent_type="tester-api", prompt="Test this REST API against its OpenAPI spec. Base URL: [URL]. Auth: [method and how to get a test token]. Endpoints: [list from spec or: cat openapi.yaml | grep '^\s*/\|^\s*get:\|^\s*post:\|^\s*put:\|^\s*delete:']. Test: happy paths, 401 without auth, 422 on invalid input, 404 on missing resources, no 500s on edge cases. Check response schemas match spec. Report: Pass/Fail per endpoint + issues.")
```

Incorporate any schema mismatches or unhandled edge cases back into the implementation.
...
→ **antes**: `plan` — arquitectura da API, decisões de versioning
→ **implementação**: `laravel-specialist` ou `nodejs` (consoante stack)
→ **após implementar endpoints**: `tester-api` — valida spec vs implementação real
→ **antes de expor publicamente**: `tester-security`

Notificar ao concluir spec: `→ próximo: laravel-specialist`
