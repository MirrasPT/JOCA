...
→ **antes**: `plan` — decisões de infra, cloud provider, estratégia de deploy
→ **após configurar pipeline**: `tester-security` — scan de secrets, CVEs, headers
→ **deploy para produção**: `deploy-forge` (Laravel) ou pipeline CI configurado
→ **após deploy**: `tester-performance` — Lighthouse + smoke test k6

Notificar ao concluir configuração: `→ próximo: tester-security`
