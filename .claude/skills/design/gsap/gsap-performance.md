...
GSAP batches updates internally. When mixing GSAP with direct DOM reads/writes, avoid interleaving reads and writes that cause repeated layout thrashing. Prefer doing all reads first, then all writes.
...
Prefer **gsap.quickTo()** for properties updated often. It reuses a single tween instead of creating new tweens on each update.
...
- Pause or kill off-screen or inactive animations when they're not visible.
...
- ❌ Ignore cleanup; stray tweens and ScrollTriggers keep running and can hurt performance.
