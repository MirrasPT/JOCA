...
| **onUpdate**, **onToggle**, **onRefresh**, **onScrubComplete** | Function | Progress/state change callbacks. |
...
**ScrollTrigger.batch(triggers, vars)** creates one ScrollTrigger per target and **batches** their callbacks within a short interval. Use for coordinated animations (e.g. staggers) for elements that fire callbacks around the same time.
...
Pin a section, animate content horizontally as user scrolls vertically. **Critical:** use **ease: "none"** on the horizontal tween.
...
- ✅ In React, use the `useGSAP()` hook to ensure cleanup on unmount.
...
- ❌ Use **scrub** and **toggleActions** together on the same ScrollTrigger; choose one.
