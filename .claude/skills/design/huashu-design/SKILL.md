...
- **演示幻灯片** → `slides` skill（单/多文件架构选型、showcase 工作流、PDF/PPTX 导出）
...
**固定尺寸内容**（动画/视频）必须自己实现JS缩放，用auto-scale + letterboxing。
...
- 动画等固定尺寸内容，**播放位置**存localStorage——刷新不丢
