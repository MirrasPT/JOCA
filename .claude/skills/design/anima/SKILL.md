---
name: anima
description: Animation specialist — GSAP for web/scroll animations and Lottie for icon/illustration/SVG animations. Routes automatically based on context. Use when adding motion to websites, animating UI elements, creating scroll-triggered sequences, building Lottie JSON animations, or exporting animations as MP4/GIF. Integrates with frontend-design and frontend-dev. References gsap/* skills for deep API docs.
triggers: animação, animation, gsap, lottie, scroll animation, page transition, hover animation, animação de ícone, animação de ilustração, scroll trigger, motion, animate, transição, efeito de entrada, animar, micro-interação, microinteraction
---
...
Quando `frontend-design` pede animação:
1. Ler DESIGN.md → `--duration-*` e `--ease-*` tokens
2. Aplicar nos GSAp defaults: `gsap.defaults({ ease: "power4.out", duration: parseFloat(getComputedStyle(root).getPropertyValue('--duration-base')) / 1000 })`
3. Só GSAP para elementos HTML; Lottie para ícones e ilustrações SVG

### Com frontend-dev

```jsx
// React — useGSAP hook (evita memory leaks)
...
### Exportar animação como MP4/GIF

Para animações HTML exportadas como vídeo, usar a skill `video` (secção HTML Animation → Video Export).

Resumo: `render-video.js` → 25fps MP4 → `convert-formats.sh` → 60fps + GIF → `add-music.sh` → BGM + SFX.
