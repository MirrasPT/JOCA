...
```bash
[ -f composer.json ] && echo "PHP/Composer" && cat composer.json | python3 -c "import json,sys; d=json.load(sys.stdin); print(f'Dependencies: {len(d.get(\"require\",{}))} require + {len(d.get(\"require-dev\",{}))} require-dev')"
[ -f package.json ] && echo "Node.js/npm" && cat package.json | python3 -c "import json,sys; d=json.load(sys.stdin); print(f'Dependencies: {len(d.get(\"dependencies\",{}))} + {len(d.get(\"devDependencies\",{}))} dev')"
...
### PHP
```bash
composer outdated --format=json 2>/dev/null | python3 -c "
import json, sys
try:
...
depcheck --json 2>/dev/null | python3 -c "
import json, sys
try:
    d = json.load(sys.stdin)
