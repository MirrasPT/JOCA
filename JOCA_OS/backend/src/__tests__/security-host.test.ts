...
// gets the victim to open http://evil.com:7491. The page then calls the API on its own origin, so
...
describe('isAllowedOrigin', () => {
  it('accepts loopback origins on any port', () => {
    expect(isAllowedOrigin('http://localhost:5173', 'localhost:7491')).toBe(true);
    expect(isAllowedOrigin('http://127.0.0.1:7491', '127.0.0.1:7491')).toBe(true);
  });

  it('accepts a call with no Origin (curl, server-side)', () => {
    expect(isAllowedOrigin(undefined, 'localhost:7491')).toBe(true);
  });

  it('rejects a cross-site origin', () => {
    expect(isAllowedOrigin('https://evil.com', 'localhost:7491')).toBe(false);
  });
  ...
  it('no longer accepts same-origin when the host itself is a stranger domain', () => {
    ...
    expect(isAllowedOrigin('http://evil.com:7491', 'evil.com:7491')).toBe(false);
  });

  it('still accepts same-origin on a host we answer to', () => {
    expect(isAllowedOrigin('http://192.168.1.10:7491', '192.168.1.10:7491')).toBe(true);
  });
});
