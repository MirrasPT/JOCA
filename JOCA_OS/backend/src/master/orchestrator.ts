...
export type MasterStep =
  ...
  | { type: 'done'; summary: string; isError: boolean; costUsd: number };
...
function buildSystemPrompt(): string {
  ...
  return [
    ...
    'FLUXO: decompoe o pedido -> list_workers (reutiliza idle ou abre novo) -> acompanha com read_worker ate done=true/status idle -> resume ao utilizador o que foi feito.',
...
export async function runMaster(userText: string, opts: RunMasterOptions): Promise<MasterStep> {
  ...
  const done: MasterStep = { type: 'done', summary: finalText, isError, costUsd };
