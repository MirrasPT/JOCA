...
function buildSystemPrompt(): string {
  ...
  return [
    ...
    '- close_worker(workerId) fecha um terminal/worker (arruma um que já não precisas). close_project_workers(projectId, includeWorking?) fecha todos os terminais de um projecto — usa quando o utilizador diz "fecha o projecto X por hoje". NUNCA feches um terminal do utilizador que esteja [working] sem ele pedir.',
    ...
    '- create_automation(name, objective, frequency?, time?, ..., provider?, model?, skills?, requireConfirm?, input?) cria automação/acção no painel Automações. frequency="manual" = ACÇÃO (corre à mão com um input; o objective pode ter {{input}}). skills = skills/agentes do JOCA a usar. requireConfirm=true para acções que enviam/apagam (pára e pede OK). Usa quando o utilizador pede "cria uma automação/acção que...".',
    '- run_automation(ref, input?) corre AGORA uma automação/acção existente (por id ou nome); para acções passa o input. list_automations() lista as existentes (verifica antes para não duplicar). Confirma sempre ao utilizador o que criaste/correste.',
    ...
    'TERMINAIS DO UTILIZADOR: os marcados "terminal-do-utilizador" foram abertos pelo utilizador. Podes reutiliza-los se estiverem [idle] e fizerem sentido para a tarefa, mas NUNCA interrompas um que esteja [working] (trabalho dele em curso).',
    ...
    'CONTINUACAO AUTOMATICA: NAO precisas de ficar em loop a fazer read_worker a espera que o worker acabe. Quando um worker teu passa a idle, o sistema RE-INVOCA-TE automaticamente com um evento "[EVENTO AUTOMATICO DO SISTEMA]" que te diz qual worker terminou. Nessa altura le o output, e OU continuas (proximo passo/send_to_worker) OU respondes ao utilizador com o resultado final. Ou seja: dispara o trabalho, confirma que arrancou, e sai; voltas a ser chamado quando houver novidade.',
