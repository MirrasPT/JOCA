...
git clone <YOUR_REPO_URL> <caminho_destino>
...
| Área                  | Skills                                                                                        |
|-----------------------|-----------------------------------------------------------------------------------------------|
| UI/UX                 | `frontend` (principal), `mobile` (responsivo), `anima` (animacao), `slides` |
...
| Deploy                | `deploy-ploi`, `deploy-cpanel`, `deploy-docker` (consoante plataforma)                        |
...
|-------------|--------------------------------------------------------|
| Laravel     | laravel-specialist, filament, mysql, rest-api, test-master |
| React       | webapp-testing, gsap/gsap-react (se animacoes)             |
...
```
Interface browser com terminal multi-sessão integrado.
Dashboard, file browser, toolkit panel, multi-session terminal.
Requer Node.js 18+.
...
Verificar: `node <caminho_joca>/../JOCA_UI/backend/dist/server.js` deve iniciar sem erros.

O JOCA_UI detecta automaticamente o JOCA_Logic como directório irmão — não requer configuração manual.

### StatusLine
```
StatusLine mostra nome do modelo, sessão e status na barra do Claude Code.
Injecto automaticamente em ~/.claude/. [S/N]
```
...
### 7. Instalar JOCA_UI (se confirmado)
...
```bash
# macOS / Linux
bash <caminho_joca>/../JOCA_UI/start.sh

# Windows
<caminho_joca>\..\JOCA_UI\start.bat
```

O backend detecta `JOCA_Logic` como directório irmão automaticamente.
Em Windows, o terminal usa `powershell.exe`; em macOS/Linux usa `$SHELL` ou `/bin/zsh`.

### 8. Injectar StatusLine (se confirmado)
