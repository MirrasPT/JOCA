...
from whisperx_local import is_available, transcribe_video  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--no-whisperx", action="store_true", help="Disable WhisperX fallback.")
    ap.add_argument(
        "--model",
        default="large-v2",
        help="WhisperX model size: tiny/base/small/medium/large-v2 (default: large-v2)",
    )
    ap.add_argument("--language", default=None, help="Force language code (e.g. pt, en, fr)")
    args = ap.parse_args()
    ...
    if not transcript_segments and not args.no_whisperx:
        if is_available():
            try:
                ...
                print(f"[watch] whisperx fallback failed: {exc}", file=sys.stderr)
        else:
            setup_py = SCRIPT_DIR / "setup.py"
            print(
                f"[watch] no subtitles and whisperx not installed — "
    ...
    if transcript_text:
        ...
        print(
            ...
            "Captions were missing and WhisperX was unavailable or disabled. "
            f"Run `python3 {setup_py}` to install WhisperX, then re-run._"
        )
