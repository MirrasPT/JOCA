...
- Quando há ficheiros em `memory/feedback/auto-*.md` (capturados automaticamente pelo /save)

## Modo Self-Improvement (opcional)

Se o utilizador diz "upgrade completo", "self-improve", ou "melhoria autónoma":
1. Invocar `Agent(subagent_type="self-improver")` com contexto do feedback acumulado
2. O self-improver analisa padrões, propõe novas skills, e valida via Gemini/Codex CLI
3. Apresenta propostas validadas ao utilizador antes de aplicar
4. Aplica após confirmação e marca tudo com `origin: local`

Este modo complementa (não substitui) a Fase 1-3 abaixo — corre APÓS o feedback manual ser processado.
...
[Se ficheiros .claude/ alterados:]
✓ Bridges recompilados (compile-bridges.sh)
✓ Skill index regenerado (build-skill-index.py)
```

### Fase 3b — Self-Improvement (se activado)

```
Invocar self-improver agent...
Propostas validadas: N (H high, M medium, L low)
[Apresentar lista de propostas]
```
