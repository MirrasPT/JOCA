...
**Para:** `master` / v2.0 (flat layout, JOCA_UI integrado, soul.md, SKILL_INDEX)
...
│   ├── tools/
│   └── feedback/
├── JOCA_UI/              ← browser UI (React + Vite + xterm.js + node-pty)
│   ├── frontend/
│   ├── backend/
│   └── data/
├── CLAUDE.md
...
**Nota:** NÃO existe pasta `JOCA_Logic/` separada. O conteúdo do JOCA_Logic vive directamente na raiz. O `JOCA_UI/backend/src/server.ts` tem `findJocaLogicRoot()` que detecta `.claude/` + `CLAUDE.md` fazendo walk-up a partir de `__dirname` — funciona com este layout sem alterações.
...
# JOCA_UI antigo (se existir — vai ser substituído)
rm -rf JOCA_UI/
...
git checkout origin/master -- .claude/ JOCA_UI/ CLAUDE.md README.md
...
```bash
cd JOCA_UI/backend && npm install && cd ../frontend && npm install && cd ../..
```
...
Próximo:
  - Lançar JOCA UI: duplo-clique em "JOCA UI.command" ou npm start em JOCA_UI/
...
- **Testar o JOCA_UI** — confirmar que `findJocaLogicRoot()` detecta o layout flat
