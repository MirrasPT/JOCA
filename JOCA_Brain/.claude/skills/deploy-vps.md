...
Padrão validado: Ubuntu VPS + Caddy v2 + Cloudflare DNS via API. Windows-first.
...
No `compose.prod.yaml`: publicar a app só em `127.0.0.1:8000:8000` (nunca `0.0.0.0`) e **não** activar o Caddy embutido. Ver `memory/projects/<your-vps>.md` (TryPost).
