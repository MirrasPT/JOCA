...
```powershell
Set-Location "<joca>\JOCA_OS\backend"
npm install
...
Set-Location "<joca>\JOCA_OS\backend"
npm run build
Set-Location "<joca>\JOCA_OS\frontend"
npm install
```

**Verify:** `node "<joca>\JOCA_OS\backend\dist\server.js"` starts without a node-pty `MODULE_NOT_FOUND` / `was compiled against a different Node.js version` error.
...
**After fixing:** if any `JOCA_OS/` source changed, rebuild (`npm run build` backend) and recompile bridges if skills/agents touched. Surface any change that could affect macOS so the user can re-verify on the reference platform.
