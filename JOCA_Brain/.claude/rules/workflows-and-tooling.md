...
- **Validar CONTEÚDO contra a fonte real (não só código).** Um worker que escreve conteúdo a partir de um site/documento (prémios, notícias, specs, copy) DEVE validar cada bloco contra a fonte e marcar `TODO: não consta da fonte` o que não existir — por defeito, não só quando o user pede comparação. A regra anti-fabricação aplica-se a conteúdo, não apenas a APIs/credenciais; sem este passo no brief, workers inventam conteúdo plausível que só é apanhado por auditoria explícita. (Fonte: bodegas-do-campo 2026-06-24.)
