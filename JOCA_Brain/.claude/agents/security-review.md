...
1. Read `.claude/skills/security.md` — OWASP Top 10:2025 + ASVS 5.0 checklist
2. Read `.claude/skills/auth.md` — auth patterns (session, Sanctum, 2FA)
3. Apply both as the reference while doing the code review
...
- Full report → write it to `.joca/intermediate/security-review-<slug>.md` (confirm `.joca/` is in the project's .gitignore; otherwise use the session scratchpad) and return to the caller only a summary ≤15 lines + the path. `Write` is in the tools only for this.
