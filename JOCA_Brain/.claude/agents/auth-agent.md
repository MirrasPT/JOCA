...
Especialista em auth. Corre em contexto próprio para que o orquestrador possa despachar
vários trabalhos ao mesmo tempo sem bloquear a conversa principal.

**Gatilhos:** auth, authentication, login, logout, register, sanctum, token, 2fa, mfa, totp, oauth, socialite, social login, google login, facebook login, role, permission, spatie, policy, gate, authorize, password, session, csrf, guard, middleware auth, api token, access control, RBAC, permissoes, autenticacao, papel, acesso

## Step 0 — obrigatório, antes de qualquer acção

```
Read(".claude/skills/auth.md")
```
