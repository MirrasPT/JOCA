---
name: tester-api
description: "Tests REST endpoints: correctness, auth, schema, IDOR, performance, edge cases. Produces a pass/fail report per endpoint. Triggers: test my API, test endpoints, API broken, HTTP 500, 401 unauthorized, test webhook, validate API response."
...
1. Read `.claude/skills/rest-api.md` — REST standards (RFC 9457 errors, pagination, OpenAPI 3.1)
2. Read `.claude/skills/auth.md` — authentication and session patterns
3. Apply these standards when validating responses (error format, status codes, auth flows)
...
### 7. Auth/API security probes (active)

> ⚠ Only against the user's/client's own apps with explicit authorization (context: the user's own apps). Never against third-party targets.

Active probes for auth/API vectors that the correctness tests do not catch:

```bash
# JWT alg:none — forge a token without a signature (must be rejected with 401)
...
# If != 401 → CRITICAL: it accepts unsigned tokens

# JWT weak-alg (HS256 forged with a trivial secret) — same rationale
# If the app uses RS256 but accepts HS256 signed with the public key → CRITICAL

# HTTP parameter pollution — does a duplicated parameter change the authorization?
curl -s -o /dev/null -w "HPP -> %{http_code}\n" \
  -H "Authorization: Bearer $TOKEN" "<URL>?user_id=<MY_ID>&user_id=<OTHER_ID>"

# SSRF in URL fields — point a URL field at metadata/loopback
curl -s -o /tmp/ssrf.json -w "SSRF -> %{http_code}\n" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"url":"http://169.254.169.254/latest/meta-data/"}' -X POST <URL>
# A response that returns internal content → CRITICAL

# Mass-assignment via an extra payload — inject privileged fields
curl -s -o /tmp/massassign.json -w "mass-assign -> %{http_code}\n" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"name":"x","role":"admin","is_admin":true,"tenant_id":999}' -X POST <URL>
# If the privileged field persists in the created resource → CRITICAL
```

Follow the same active-probe pattern as `tester-ratelimit`: send the real malicious request, compare the status/response against the expected one, report with a reproducible PoC.
...
- Full report → write it to `.joca/intermediate/tester-api-<slug>.md` (confirm `.joca/` is in the project's .gitignore; otherwise use the session scratchpad) and return to the caller only a summary ≤15 lines + the path.
