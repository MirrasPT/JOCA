...
name: gemini-brain
description: |
  Multimodal or 1M-token-context tasks via `agy` (Antigravity CLI / Google Gemini):
  analyzing video (YouTube, direct links), local video/audio files, large PDFs,
  or any content that exceeds Claude's context. Triggered by: "analyze with Gemini",
  "too long for context", "second opinion from Gemini", video/PDF/audio analysis when cloud is
  acceptable. Different from gemini-auditor (verification/audit of JOCA components and code by a
...
Check if `agy` is installed and authenticated:

```bash
agy --version 2>/dev/null || echo "AGY_NOT_INSTALLED"
```

**If not installed**: requires `agy` (Antigravity CLI) on the PATH. If missing: report to the user and stop — do NOT try to install it yourself.

**If installed**, verify authentication silently by running a minimal test:

```bash
agy -p "hi" --output-format json 2>&1 | head -5
```

If the output contains auth errors or "not authenticated", guide the user through `agy auth login` and stop.
...
```bash
cat "PATH_TO_FILE" | agy "USER_PROMPT_HERE"
```
...
If the response is very long (>3000 words), summarize key sections and note that the full output is available on request.

## Step 4 — Note model tier

At the end of the response, add one line identifying the tool + version, **derived from the runtime — never hardcode the version or model name** (they age):

```bash
echo "[agy (Antigravity CLI) $(agy --version 2>/dev/null) — model per agy config]"
```
