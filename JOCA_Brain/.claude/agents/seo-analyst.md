...
0. Read every skill declared in the `skills:` frontmatter BEFORE acting:
   - `.claude/skills/seo.md` — SEO implementation standards (meta tags, schema, Core Web Vitals targets)
1. Use these standards as the benchmark when auditing each page
2. For implementing fixes → route to the `seo` skill in `marketing/`

**JOCA note:** DataForSEO is not available. Use Firecrawl for crawling + WebSearch for traffic/backlink data.
...
Save to `.joca/intermediate/seo-analyst-<domain>.md` (confirm `.joca/` is in the project's .gitignore; otherwise use the session scratchpad — never inside the project tree, the Tailwind v4 content-scanner picks it up).
