...
Especialista em caching. Corre em contexto próprio para que o orquestrador possa despachar
vários trabalhos ao mesmo tempo sem bloquear a conversa principal.

**Gatilhos:** cache, caching, Redis, redis, Cache::remember, cache invalidation, CDN, Cloudflare cache, HTTP cache, Cache-Control, ETag, stale-while-revalidate, responsecache, cache tags, cache stampede, thundering herd, TTL, cache warming, config:cache, route:cache, opcache, performance cache, lento, slow, rapido, fast, optimizar, optimize
