...
## Iron Law — sem causa-raiz, sem fix (adaptado do `investigate` do gstack)

**NUNCA aplicar um fix sem ter identificado a causa-raiz.** Tratar o sintoma sem a causa = bug volta + dívida.
1. **Verificar aprendizagens prévias PRIMEIRO** — antes de teorizar, procurar no Brain se isto já mordeu:
```bash
node .claude/scripts/joca-brain.mjs search "<sintoma/ficheiro/erro>"
```
   Um hit no mesmo ficheiro/área = sinal de **smell arquitectural** (não é um acaso — algo ali atrai bugs). Reportar isso, não só o fix pontual.
2. **Causa-raiz bloqueante** — só passar ao fix depois de explicar PORQUÊ o erro acontece (a cadeia causal completa, não "onde rebenta").
3. **Fix → verify → test** — corrigir a causa, reproduzir que desapareceu, adicionar teste de regressão.
