...
1. Read `.claude/skills/error-tracking-dev.md` — available debug tools (Debugbar, Telescope, Ray, Clockwork, Pail)
2. If you find slow queries → suggest `query-debugger` as a follow-up
3. If you find recurring cross-service patterns → suggest `error-detective` (built-in agent)
...
# Unique error messages (normalized)
...
## Iron Law — no root cause, no fix (adapted from gstack's `investigate`)

**NEVER apply a fix without having identified the root cause.** Treating the symptom without the cause = the bug comes back + debt.
1. **Check previous learnings FIRST** — before theorizing, search the Brain for whether this has bitten before:
```bash
node .claude/scripts/joca-brain.mjs search "<symptom/file/error>"
```
   A hit in the same file/area = a sign of an **architectural smell** (it is not chance — something there attracts bugs). Report that, not just the one-off fix.
2. **Root cause is blocking** — only move to the fix after explaining WHY the error happens (the full causal chain, not "where it blows up").
3. **Fix → verify → test** — fix the cause, reproduce that it is gone, add a regression test.
4. **Escalate after 3 failed attempts** — if 3 root-cause hypotheses do not confirm, stop trying fixes and report the hypotheses + what is missing to tell them apart (do not hammer blindly).
5. **Record the lesson** — a non-obvious bug that would come back → `node .claude/scripts/joca-brain.mjs learn --text "<cause+fix>" --tags bug,<area>` (feeds recall and future `search`).

Full report → write it to `.joca/intermediate/log-debugger-<slug>.md` (confirm that `.joca/` is in the project's .gitignore; otherwise use the session scratchpad) and return to the caller only a summary of ≤15 lines + the path.

## Next step (chain)
- The cause is SQL / a slow query → `query-debugger`. Fix applied → `tester-code` (re-verify). See `rules/chaining.md`.
