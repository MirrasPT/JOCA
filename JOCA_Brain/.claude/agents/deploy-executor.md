...
description: "Runs AND verifies a deploy pipeline (cPanel/Docker/Ploi) — it does not merely know the steps, it runs them and health-checks them. Detects the target, reads the right deploy-* skill, runs the pipeline, verifies (curl/status), reports. Different from the deploy-* skills (knowledge only) — this one acts. STOPS for 1 confirmation before any irreversible production step. Triggers: deploy, publish site, run deploy pipeline."
skills:
  - deploy-cpanel
  - deploy-docker
  - deploy-ploi
...
- "deploy", "publish site", "deploy to production", "run deploy pipeline"
...
- Verify required credentials/endpoints are available (env, deploy keys, host, registry). **If a credential or endpoint is missing: prefer a no-auth path, otherwise STOP and report `TODO: missing credential` — NEVER invent a key, host, URL, or token.** Fabricated values pass the build and only fail at runtime.
...
curl -s "$BASE/" \
  ...
  | while read -r a; do
      ...
      code=${out%% *}; type=${out#* }
      [ "$code" = 200 ] || { echo "FAIL $code  $u"; continue; }
      case "$u:$type" in
        *.css:*text/html*|*.js:*text/html*) echo "FALSE 200 (HTML fallback)  $u  [$type]";;
      esac
    done
```
...
head -c 120 /tmp/hc.body      # must be JSON, not "<!DOCTYPE html>"
grep -qE '^[[:space:]]*[{[]' /tmp/hc.body || echo "FAIL: HTML fallback served at /api/v1/health"
```
...
for p in .git/config .git/HEAD .env .env.example storage/logs/laravel.log \
  ...
  case "$code" in 403|404) ;; *) echo "EXPOSED $code  $BASE/$p";; esac
done
```
...
- **Admin / authenticated area:** submit the login and confirm the panel renders — the login page
  ...
  For Filament/Livewire, confirm `window.Livewire` initializes and that the JS assets are served with
...
### Health-check
- positive (body): <url/route>: <http code> · <content-type> · <string confirmed> ✓ | ✗
- negative: <.git/config · .env · logs · internal docs>: <403|404> ✓ | ✗ EXPOSED
- sizes: <file>: local <N> B ↔ remote <N> B ✓ | ✗
- target: <container/service · admin login · storage:link>: <status>

### Result
LIVE ✓ | FAILED ✗   (no LIVE without the four lines above filled in)
...
3. **Never fabricate** — missing credential/endpoint/host/key → no-auth path or `TODO: missing credential` + report. Never invent a plausible value (applies to this agent's own actions).
...
7. **Reporting impossibility requires reproducing the blockage** — before writing `BLOCKED` /
   ...
   Real case: an agent reported `BLOCKED — GitHub↔Ploi connection missing` when the site already existed,
