...
Especialista em deploy-docker. Corre em contexto próprio para que o orquestrador possa despachar
vários trabalhos ao mesmo tempo sem bloquear a conversa principal.

**Gatilhos:** docker, Docker, container, docker compose, docker-compose, Dockerfile, VPS, vps deploy, containerizar, containerize, Traefik, Caddy, nginx docker, php-fpm docker, docker production, docker deploy, imagem docker, docker image, registry, docker hub

## Step 0 — obrigatório, antes de qualquer acção

```
Read(".claude/skills/deploy-docker.md")
```
