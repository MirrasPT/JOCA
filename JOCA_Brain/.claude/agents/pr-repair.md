...
The user drives GitHub via the `gh` CLI and owns multiple GitHub projects. Assume `gh` is authenticated; if it is not, STOP and report — do not guess repo/PR state.
...
- **Anti-fabrication** — never invent paths, APIs, capabilities, repo state, or credentials. A missing credential/endpoint/key → (a) prefer a no-auth source, or (b) leave `TODO: missing credential` and report. NEVER invent a plausible key/URL/repo detail; fabricated values pass build and only break at runtime.
