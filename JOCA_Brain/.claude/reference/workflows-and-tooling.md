...
- **Validar CONTEÚDO contra a fonte real (não só código).** Um worker que escreve conteúdo a partir de um site/documento (prémios, notícias, specs, copy) DEVE validar cada bloco contra a fonte e marcar `TODO: não consta da fonte` o que não existir — por defeito, não só quando o user pede comparação. A regra anti-fabricação aplica-se a conteúdo, não apenas a APIs/credenciais; sem este passo no brief, workers inventam conteúdo plausível que só é apanhado por auditoria explícita. (lição de projecto anterior.) **Estende-se a ELEMENTOS VISUAIS de brand:** ao iterar sobre um design a partir de referências, NÃO inventar elementos não presentes nas refs (barras, labels, watermarks, badges, slogans). Se não está visível na referência → não existe. (lição de projecto anterior — inventou elementos visuais nunca presentes nas referências.)
...
O utilizador corre **Windows** como ambiente primário. Ao escrever scripts/skills que tocam credenciais, binários ou paths:
