...
- 2026-06-28: **REVERSÃO do estilo de arte — manga → pintura semi-realista (estilo 5).** Renato rejeitou o manga; sweep de estilos confirmou preferência por splash-art pintada (cinematic, painterly, artstation). **21 Egípcios já regenerados nesse estilo e LIVE**; as outras 4 facções (84 cartas) ainda em manga — decisão pendente de unificar. Substitui a decisão de 2026-06-27.
...
- **Unificar estilo de arte:** Egípcios já em estilo 5 (pintura semi-realista); decidir se regenera as outras 4 facções (84 cartas) p/ casar — senão o catálogo fica com 2 estilos (manga + pintura). Pipeline pronto (`style_sweep.ps1` + JSON por carta).
- **Integrar arte no Unity:** levar os PNG de `assets/cards/` para a UI da carta (moldura + nome + stats por cima da ilustração). Gerar `leg_padeira` se quiser a lenda neutra.
...
## Última sessão
2026-06-28 (g) — **Ajustes de Design de Etiquetas, Cores Semânticas Fixas e Correções.** Fixadas cores semânticas para as caixas de etiquetas de trigger/keywords, independentes da facção. Corrigida a regex para separar gatilhos `Activar (X)` de custos personalizados (ex.: Sacrifica uma unidade). Alterada a habilidade da *Lança Dory* para ganho de gold em vez de redução de custo.

2026-06-28 (f) — **Pivô de estilo de arte: manga → pintura semi-realista (estilo 5), só Egípcios (21) regenerados + redeploy.** Renato rejeitou a arte manga/cel-shaded ANIMA. Sweep de ~13 estilos (mesmo sujeito Cleópatra, mesma seed 555000, prompt por-item sem preâmbulo fixo) → escolhido **estilo 5 = "semi-realistic digital painting, richly rendered fantasy splash art, cinematic dramatic lighting, painterly brushwork, trading card game key art, artstation quality"**. Regeneradas as **21 cartas egípcias** (1 General + 20) nesse estilo via ANIMA (`anima-base-v1.0` + `anima-turbo-lora`, 896×1152, seeds 600001+). Deploy: scp p/ VPS + **purga obrigatória da cache Cloudflare** (senão serve antigo). **Cajado Was + Mumificação** saíram com moldura de carta → regeneradas full-bleed (cenário a encher os 4 cantos + negativos anti-moldura equilibrados). User commitou tudo + rebrand para **OmniClash** (commit `ff420b7`), working tree limpo e pushed. **Próximo natural:** decidir se o estilo 5 substitui o manga nas outras 4 facções (84 cartas).
