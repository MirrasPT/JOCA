...
- ⚠ **TikTok BLOQUEADO até auditar a app (confirmado 2026-06-27):** o 1º TikTok agendado (clip 05-AMesmaCancao-B, 27 Jun 18:30) falhou — `status: failed`, erro TikTok `unaudited_client_can_only_post_to_private_accounts` (TryPost: "App not approved for public posting"). **O bloqueio é da CONTA, não do post:** app não-auditada só publica em conta TikTok em modo **privado** — `SELF_ONLY` também falha porque `@simaosinapt` é pública. **TryPost só faz Direct Post** (`TikTokPublisher.php`: `/post/publish/video/init/` + `post_mode DIRECT_POST`); **NÃO tem caminho inbox/draft** → não dá para enviar como rascunho para a app o user agendar à mão. Vias: (a) **auditar a app TikTok** (Content Posting API → Direct Post, ~dias) → único modo p/ conta pública; (b) upload **manual** do mp4 na app; (c) patch ao TryPost p/ inbox-upload. **Decisão (Renato): publicação TikTok fica MANUAL na app por agora.** IG não afectado (publica normal). Nota TryPost: post `failed` está finalizado (`posts.cannot_edit_finalized`) → re-tentar = criar post novo + reanexar media.
- 🔎 **Sandbox Content Posting API — VEREDICTO DEFINITIVO (2026-06-29, docs oficiais TikTok):** investigado a fundo. **Sandbox + Target User + scope `video.publish` autorizado NÃO chega para publicar público.** Screenshots provam: `@simaosinapt` JÁ era Target User desde 27 Jun 12:01 (e `video.publish`="Directly post content" está disponível) — e mesmo assim o post falhou às 18:30. **→ A minha hipótese anterior (causa = não ser Target User) está ERRADA.** Causa real, confirmada na doc: **app NÃO-AUDITADA está limitada a `private viewing mode`** independentemente do sandbox/target user. Citações oficiais (Content Posting API · Get Started + Direct Post reference):
  - "All content posted by unaudited clients will be restricted to **private viewing mode**. Once you have successfully tested your integration, to lift the restrictions on content visibility, your API client must undergo an **audit**."
  - Erro `unaudited_client_can_only_post_to_private_accounts`: *"Unaudited clients can only post to a **private account**. The publish attempt will be **blocked when calling `/publish/video/init/`**."*
  - Unaudited → só `SELF_ONLY`; `PUBLIC_TO_EVERYONE`/`MUTUAL_FOLLOW_FRIENDS` indisponíveis até auditar.
  **Tradução prática:** (a) **publicar público = impossível sem auditoria de produção**, ponto final; (b) para o sandbox publicar SEQUER (mesmo SELF_ONLY) a **conta TikTok tem de estar em modo PRIVADO** (Definições→Privacidade→Conta privada) — como `@simaosinapt` é pública, o init é bloqueado → é exactamente o erro de 27 Jun; (c) Target User = necessário mas **não suficiente**. **Único valor do sandbox:** validar o pipeline do TryPost + **gravar o demo video** para a submissão (pôr conta privada → post de teste sai SELF_ONLY, visível só ao dono → repor pública). Zero alcance. Distribuição pública real → **auditoria da app de produção** ou manual. Walkthrough citado pelo user (https://www.youtube.com/watch?v=_XgsVpggsSY) é só o flow de setup, não contorna a restrição de visibilidade.
...
- **TikTok DESAGENDADO do TryPost (2026-06-29):** confirmado que sandbox não dá alcance público por nenhuma via (Direct Post→SELF_ONLY; inbox/draft não entrega no sandbox). Desliguei a plataforma TikTok dos **14 posts conjuntos** (5→18 Jul) — ficaram **só Instagram Reel** (IG intacto, 27 posts IG agendados). 4 clips (27/29 Jun, 1/3 Jul) tinham sido publicados como SELF_ONLY privados (apagar no app). **Acção p/ retomar TikTok:** (a) **App Review de produção** (form preenchido; falta só o **demo video**) → desbloqueia inbox/draft (leve) + direct post; OU (b) **upload manual** no app. Ver secções "TikTok via sandbox" + "DRAFT/inbox".
...
## Workflow CONFIRMADO: publicar no TikTok via sandbox (privado/SELF_ONLY) — 2026-06-29
Com a conta `@simaosinapt` em **modo Privado** (Definições→Privacidade) + app sandbox + Target User, o **Direct Post com `privacy_level: SELF_ONLY` PUBLICA com sucesso** via TryPost (testado e2e, 4 clips `published`, `platform_url` devolvido). Não é draft — é post **privado** (visível só ao dono). Serve para validar pipeline + **gravar o demo video** da auditoria. **Zero alcance público** (continua a precisar de auditoria de produção para público).
- **Posts `failed`/finalizados** (ex.: o de 27 Jun PUBLIC) não se editam (`posts.cannot_edit_finalized`) → **recriar**: `create-post` (tiktok, `meta.privacy_level=SELF_ONLY`) → `attach-media-from-url` (reusa o storage URL antigo `trypost.rfdev.pt/storage/medias/<id>.mp4`) → `publish-post`.
- **Posts `scheduled`** → `update-post` (passar só a plataforma tiktok com `meta.privacy_level=SELF_ONLY`; IG já estava `enabled=false` em todos os clips TikTok, logo não se mexe no IG) → `publish-post` (sem `scheduled_at` = imediato).
- TikTok demora ~20-60s a processar (status `publishing`→`published`); confirmar por `get-post`.
- ⚠ Ao reabrir a conta a pública, estes 4 ficam SELF_ONLY na mesma (privacy é por-post). Para os tornar públicos depois da auditoria há que republicar.

## ⚠ TikTok DRAFT/inbox via raw API — NÃO ENTREGA NO SANDBOX (2026-06-29, testado e2e)
**Conclusão corrigida:** o fluxo inbox/draft (`video.upload`, `/v2/post/publish/inbox/video/init/`) **aceita a chamada e o TikTok descarrega o ficheiro inteiro, MAS fica preso em `PROCESSING_DOWNLOAD` indefinidamente (80+ min) e NUNCA entrega ao inbox/app** — porque a app TryPost está em **SANDBOX** (`TIKTOK_CLIENT_ID=sbaw…`). Testado nos 4 clips por `PULL_FROM_URL`: todos `error.code:ok`, `downloaded_bytes == content-length` (download completo), status nunca avança. Nada aparece (nem em Drafts nem em notificações). `FILE_UPLOAD` deu PUT 404 ao upload-server (não resolvido). **Direct Post (SELF_ONLY) FUNCIONA no sandbox, mas inbox/draft NÃO.** → O inbox/draft só entregará com a **app em PRODUÇÃO** (passar o App Review). Ou seja: nem o caminho "rascunho" escapa à submissão da app. Para alcance público continua: **(a) App Review de produção** (form + demo video) e depois inbox/draft (`video.upload`, sem a auditoria estrita de direct-post-público) OU direct post auditado; **(b) upload MANUAL no app**. Mecânica raw (válida p/ produção): decriptar token via `artisan tinker` (`SocialAccount->access_token`, cast `encrypted`), correr na VPS; `upload_url` traz `&` (decodificar p/ `&`).
<!-- nota histórica: o fluxo inbox/draft em PRODUÇÃO não precisa de conta privada e deixa publicar público com 1 toque no app; só transfere o vídeo, caption escreve-se no app. Mas exige app live. -->
DETALHE OBSOLETO (mantido só p/ referência da mecânica):
- **Pré-req:** token com grant `video.upload`. O token guardado pelo TryPost para `@simaosinapt` JÁ tem todos os scopes (`user.info.basic/profile/stats, video.list, video.publish, video.upload`).
- **Token está ENCRIPTADO na BD** (Laravel cast `encrypted`): `select access_token from social_accounts` devolve ciphertext → TikTok responde `access_token_invalid`. Decriptar via app container: `docker exec trypost php artisan tinker --execute='echo \App\Models\SocialAccount::where("platform","tiktok")->first()->access_token;'` (devolve `act.…`, len ~72). **Correr a chamada NA VPS** para o token não sair de lá.
- **PULL_FROM_URL** com os storage URLs do TryPost (`trypost.rfdev.pt/storage/medias/<id>.mp4`, públicos 200) funciona — domínio já verificado no TikTok. Body: `{"source_info":{"source":"PULL_FROM_URL","video_url":"<url>"}}`. Resposta OK = `{"data":{"publish_id":"v_inbox_url~v2…"},"error":{"code":"ok"}}`. Estado via `/v2/post/publish/status/fetch/` (`PROCESSING_DOWNLOAD`→`SEND_TO_USER_INBOX`).
- VPS: `root@194.62.248.50` (key `~/.ssh/datalix_id`), containers `trypost`/`trypost-pgsql`/`trypost-redis`, DB `trypost`/user `trypost`.
- ⚠ Token expira ~24h (era `2026-06-30 09:00`) — re-conectar no TryPost renova. **TryPost NÃO expõe inbox** (só Direct Post) → este caminho é raw API à parte.

## Última sessão
2026-06-29 (cont.3) — **TikTok desagendado do TryPost.** Desliguei a plataforma TikTok dos 14 posts conjuntos (5→18 Jul) → ficaram só IG Reel (verificado: 0 posts agendados com TikTok, 27 com IG). Decisão: TikTok da campanha sai do TryPost até App Review de produção (falta só demo video) ou upload manual.
2026-06-29 (cont.2) — **TikTok DRAFT via raw API inbox: 4 clips enviados aos Drafts da app** (Heranca-A, EventoPerdido-A, Recreio-B, AMesmaCancao-B), todos `error.code:ok` + publish_id, a transferir (`PROCESSING_DOWNLOAD`). Caminho público-sem-auditoria validado e2e. Falta o user abrir o app → notificação/Drafts → escrever caption → publicar público. (Estes 4 já tinham sido publicados antes como SELF_ONLY privados — há duplicação; os privados podem apagar-se no app.)
2026-06-29 (cont.) — **Publicados 4 clips no TikTok como SELF_ONLY (privado)** via TryPost, conta em modo privado: 06-Heranca-A, 03-EventoPerdido-A, 01-Recreio-B, 05-AMesmaCancao-B (recriado, o de 27 Jun que falhara). Todos `published`. Validou o pipeline e o método SELF_ONLY+conta-privada. Próximo: usar isto para gravar o demo video da auditoria de produção.
2026-06-29 — **TikTok app review + reconciliação sandbox.** Form de submissão da app de PRODUÇÃO ("Trypost Joca", client key `awi3dwc1wa3cvouk`) preenchido (description, ToS/Privacy reais em `trypost.rfdev.pt/{terms,privacy}`, scopes `user.info.basic`+`video.publish`, products Content Posting API). Assets localizados: ícone `JOCA_Drops/trypost-icon/post-icon.png` (1024×1024 ✓), páginas legais live na VPS. **Falta só o demo video** (nunca gravado). Info nova do user sobre sandbox: Direct Post sem review funciona MAS só p/ Target Users + vídeo-only + provável SELF_ONLY (sem alcance público). Causa raiz do fail de 27 Jun = `@simaosinapt` não estava como Target User. Conclusão: sandbox testa o pipeline mas não dá público; público = auditoria de produção ou manual.
2026-06-27 (noite) — **Triage publicação TikTok:** confirmado que o TikTok está bloqueado até auditar a app (`unaudited_client_can_only_post_to_private_accounts`; conta pública recusa até `SELF_ONLY`). TryPost só faz Direct Post (sem inbox/draft). TryPost UP (containers healthy na VPS). IG deste post (05-AMesmaCancao cap.5) já publicado pelo Renato. **Decisão: TikTok fica para publicação manual na app; adiar auditoria p/ amanhã.**
