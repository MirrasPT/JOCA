...
- **Ficha "90 % → 100 % castas do Douro" (2026-06-28):** corrigido em PDF + `build_ficha.py` (gerador) + `FICHA_TECNICA_...Reserva_2019.md`. 100% correcto (Touriga Franca 45 + Touriga Nacional 45 + Tinta Roriz 10, todas castas do Douro). PDF redeployed via SFTP para `royaldouro.com/ficha-tecnica.pdf`; CF cache já `EXPIRED`+revalidada (não precisou `?v=N`). Linha do destaque = `<li>...castas do Douro</li>` em `build_ficha.py`.
- **Gerador da ficha agora DURÁVEL em `_Email/` (2026-06-30):** `build_ficha.py` + `ficha_assets.py` (base64 SIDE/LOGO) copiados da scratchpad efémera para `G:\...\Royal Douro\_Email\` (já não dependem de `%TEMP%`). Correr de dentro de `_Email/` (`exec(open("ficha_assets.py"))` + `ficha_v2.html` são relativos; output hard-coded para `_Email/Ficha_Tecnica_Royal_Douro.pdf`). Renderiza via Chrome headless `--print-to-pdf`.
- **Ficha — limpeza de disclaimer + rodapé (2026-06-30):** (1) removida a nota `* Valores analíticos estimados e provisórios...certificadoras` + o asterisco órfão no cabeçalho "Análise" (o `margin-top:auto` que empurrava o rodapé para o fundo passou para o `.foot`); (2) rodapé reduzido de `Royal Douro · royaldouro.com · +351 911 156 080 · geral@vinartis.pt` para **só `royaldouro.com · +351 911 156 080`** (sem nome nem email). PDF regenerado + redeployed via SFTP + **CF cache purgada via API** (zone `royaldouro.com` = `5967e20d504ebab2dbe23bf507baa5fc`, token `~/.cloudflare/datalix.json` tem acesso a esta zona, não só rfdev.pt). Live confirmado HTTP 200 / `application/pdf` / `Cf-Cache-Status: MISS`. PDF final = 281272 bytes.
...
## Última sessão
2026-06-30 — ficha técnica: removido disclaimer de valores provisórios + asterisco órfão; rodapé reduzido a site+número (sem "Royal Douro"/email). Gerador movido para `_Email/` (durável). Redeploy SFTP + purge CF via API (zone royaldouro.com); live confirmado.
2026-06-28 — correcção ficha técnica "90 % → 100 % castas do Douro" (PDF + gerador + markdown) + redeploy do PDF via SFTP para royaldouro.com (CF revalidou sozinha).
