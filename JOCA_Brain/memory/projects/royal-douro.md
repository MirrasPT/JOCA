...
- **Ficha "90 % → 100 % castas do Douro" (2026-06-28):** corrigido em PDF + `build_ficha.py` (gerador) + `FICHA_TECNICA_...Reserva_2019.md`. 100% correcto (Touriga Franca 45 + Touriga Nacional 45 + Tinta Roriz 10, todas castas do Douro). PDF redeployed via SFTP para `royaldouro.com/ficha-tecnica.pdf`; CF cache já `EXPIRED`+revalidada (não precisou `?v=N`). ⚠ **Gerador da ficha = `build_ficha.py` + `ficha_assets.py` (base64 SIDE/LOGO) na scratchpad da sessão de 2026-06-27** (`%TEMP%\claude\...\62c0304f...\scratchpad`) — frágil (scratchpad é efémero). Mover para `_Email/` se preciso regenerar no futuro. Linha do destaque = `<li>...castas do Douro</li>` em `build_ficha.py`.
