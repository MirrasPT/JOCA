...
with open(r"<YOUR_COMFYUI_DIR>\ComfyUI\user\default\workflows\my_workflow_api.json") as f:
...
**Workaround (Windows portable install at `<YOUR_COMFYUI_DIR>`):**

```powershell
Start-Process `
  -FilePath "<YOUR_COMFYUI_DIR>\python_embeded\python.exe" `
  -ArgumentList "-s ComfyUI\main.py --windows-standalone-build" `
  -WorkingDirectory "<YOUR_COMFYUI_DIR>"
